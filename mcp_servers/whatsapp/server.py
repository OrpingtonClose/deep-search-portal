"""WhatsApp MCP Server — searches WhatsApp-related public content.
...
mcp = FastMCP("whatsapp-search")


@mcp.tool()
async def whatsapp_search(query: str) -> str:
    """Search WhatsApp group invites and public references.
    ...
    try:
        raw = await search_providers.multi_search_site(
            query, "chat.whatsapp.com", max_results=10
        )
    ...
    if not results_all:
        return f"No WhatsApp results for: {query}"
    ...
    return (
        _format_search_results(unique[:15], source_label="whatsapp")
        or f"No WhatsApp results for: {query}"
    )


if __name__ == "__main__":
    mcp.run(transport="sse", port=int(os.getenv("MCP_WHATSAPP_PORT", "9835")))
